...
(async()=>{
 ...
 try{
  for(const width of [1280,390]){
   ...
   await page.goto(process.env.LAB_URL || 'http://localhost:8521');
   ...
   await selector.click();await selector.fill('PLC点动与长动的逻辑学习');
   ...
   for(const [start,stop,jog,hold] of [[true,false,true,true],[false,false,false,true],[false,true,false,false],[false,false,false,false]]){
    for(const [label,value] of [['S：启动请求',start],['T：停止请求',stop]]){
    ...
    await page.getByText('点动结果：'+(jog?'真':'假'),{exact:true}).waitFor();
    await page.getByText('长动结果：'+(hold?'真':'假'),{exact:true}).waitFor();
    await page.waitForTimeout(400);
   }
   await page.getByRole('heading',{name:'对比互动：点动与长动',exact:true}).scrollIntoViewIfNeeded();
   const check=async(name)=>{
    if(await page.getByTestId('stException').count())throw Error('App exception');
    if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Horizontal overflow');
    await page.screenshot({path:path.join(os.tmpdir(),`dzt-v418-${name}-${width}.png`)});
   };
   await check('logic');
   const solve=async(n)=>{
    for(let i=1;i<=n;i++){
