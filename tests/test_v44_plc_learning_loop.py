...
def test_four_plc_units_have_five_traceable_questions_and_one_case():
    ...
    for unit, chapter_id, case_id in zip(units, PLC_CHAPTERS, PLC_CASES):
        ...
        assert len(questions) == (6 if chapter_id == "p2_unit_1" else 8 if chapter_id == "p2_unit_2" else 5)
...
def test_v44_ui_and_counts_are_updated():
    ...
    assert 'APP_VERSION = "4.12"' in config and 'UI_STATE_VERSION = "4.12"' in app
    assert len(QUESTIONS) == 130 and len(DIAGRAM_CASES) == 22
