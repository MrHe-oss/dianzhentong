...
(async () => {
  ...
  try {
    for (const width of [1280,390]) {
      ...
      await chapter.click();
      const unit = process.env.UNIT_SEARCH || '硬件';
      await chapter.fill(unit);
      await page.getByRole('option').filter({hasText:unit}).click();
      await page.getByText(process.env.EXAMPLE_TITLE || '🧩 原创例题：硬件角色分类',{exact:true}).click();
