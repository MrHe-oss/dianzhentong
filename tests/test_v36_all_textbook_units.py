...
def test_all_three_textbook_units_have_complete_lessons():
    ...
    assert len(TEXTBOOK_LESSONS) == 18
...
def test_v36_shows_unit_progress_and_project_summary():
    ...
    assert 'APP_VERSION = "3.6"' in config and 'UI_STATE_VERSION = "3.6"' in app
    for phrase in ("项目1学习进度", "项目1知识学习", "三个单元的知识小课已完成", "selected_textbook_chapter"):
