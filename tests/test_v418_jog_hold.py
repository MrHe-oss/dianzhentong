...
from dianzhentong.jog_hold import TOPICS, compare_jog_hold, QUESTION_SPECS
...
BOOK, UNIT = 'electrical_control_plc_s71200_tong', 'p3_unit_3'


def test_exhaustive_transitions():
    for p, stop, s, old in product((False, True), repeat=4):
        result = compare_jog_hold(p, stop, s, old)
        assert result['jog'] == (p and not stop and s)
        assert result['next_hold'] == (p and not stop and (s or old))
        assert result['reason']
        assert result['jog'] == compare_jog_hold(p, stop, s, not old)['jog']
    with pytest.raises(ValueError):
        compare_jog_hold('false', False, False, False)


def test_start_release_stop_release_sequence():
    old = False
    for stop, start, expected in [(False, True, (True, True)), (False, False, (False, True)), (True, False, (False, False)), (False, False, (False, False))]:
        result = compare_jog_hold(True, stop, start, old)
        assert (result['jog'], result['next_hold']) == expected
        old = result['next_hold']


def test_content_and_independent_pool():
    unit = load_textbook_content(BOOK, 'project_3')['project']['units'][2]
    ...
    pool = textbook_question_pool((UNIT,), 'jog_hold_example')
    assert len(pool) == 7 and 'jog_hold_example' not in {q.id for q in pool}
    ...
    assert unit['worked_example']['practice_answer'] == QUESTION_MAP['jog_hold_example'].answer
...
def test_archive_progress_and_no_old_record_changes(tmp_path, backend):
    ...
    pool = textbook_question_pool((UNIT,), 'jog_hold_example')[:5]
    ...
    q = QUESTION_MAP['jog_hold_example']
...
def test_ui_unit_complete_flow(tmp_path, monkeypatch, answer_kind):
    ...
    app.selectbox(key='selected_textbook_chapter').set_value(9).run()
    assert not app.exception
    repo = PracticeRepository(db)
    before = repo.export_snapshot()
    assert app.session_state['jog_hold_unit_hold'] is False
    app.checkbox(key='jog_hold_unit_start').check().run()
    assert app.session_state['jog_hold_unit_hold'] is False
    for start, stop, jog, hold in [(True, False, True, True), (False, False, False, True), (False, True, False, False), (False, False, False, False)]:
        app.checkbox(key='jog_hold_unit_start').set_value(start).run()
        app.checkbox(key='jog_hold_unit_stop').set_value(stop).run()
        app.button(key='jog_hold_unit_calculate').click().run()
        result = app.session_state['jog_hold_unit_evidence'][-1]
        assert (result['jog'], result['next_hold']) == (jog, hold)
    app.button(key='jog_hold_unit_reset').click().run()
    assert app.session_state['jog_hold_unit_hold'] is False
    assert 'jog_hold_unit_evidence' not in app.session_state
    app.session_state['jog_hold_unit_hold'] = 'invalid-old-state'
    app.run()
    assert app.session_state['jog_hold_unit_hold'] is False
    assert repo.export_snapshot() == before
    app.button(key='book_pretest_start_9').click().run()
    ...
    app.button(key='book_quiz_start_9').click().run()
    ids = app.session_state['quiz_state']['question_ids']
    assert len(ids) == 5 and 'jog_hold_example' not in ids
    for i, qid in enumerate(ids):
        ...
        if i == 0:
            ...
            assert app.session_state['textbook_context']['chapter_index'] == 9
    ...
    next(b for b in app.button if b.label == '返回教材单元').click().run()
    assert app.session_state['selected_textbook_chapter'] == 9
