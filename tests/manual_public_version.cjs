...
(async()=>{
 ...
 try {
  ...
  if(!(await page.locator('body').innerText()).includes('4.17')) throw Error('Expected v4.17');
  ...
  await chapter.click(); await chapter.fill('PLC正反转控制的逻辑学习');
  ...
  console.log(JSON.stringify({url:page.url(),version:'4.17',unit:true,pool:7}));
 } catch(error) {
