...
from dianzhentong.content_loader import ContentValidationError, load_textbook_content, validate_textbook_content
...
def test_structured_content_drives_catalog_and_all_lessons():
    ...
    topic_ids = [topic["id"] for unit in content["project"]["units"] for topic in unit["topics"]]
    assert topic_ids == list(TEXTBOOK_LESSONS)
    assert len(topic_ids) == 18
