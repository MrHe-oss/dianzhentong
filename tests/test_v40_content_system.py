...
def test_structured_content_drives_catalog_and_all_lessons():
    ...
    assert set(topic_ids) <= set(TEXTBOOK_LESSONS)
    assert topic_ids == list(TEXTBOOK_LESSONS)[:len(topic_ids)]
    assert len(topic_ids) == 33
...
def test_v40_has_authoring_guide_and_keeps_ui_data_components_safe():
    ...
    assert 'APP_VERSION = "4.16"' in config and 'UI_STATE_VERSION = "4.16"' in app
