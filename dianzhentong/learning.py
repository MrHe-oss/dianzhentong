...
KNOWLEDGE_CARDS: dict[str, dict[str, str]] = {
    ...
    "plc_program_cycle": {"title":"PLC程序循环","principle":"教学模型按本轮读取的信息—本轮计算—结果更新理解循环。","role":"区分外部输入、已读取快照和更新结果。","normal":"能解释读取后变化为何不改变本轮快照。","abnormal":"把抽象阶段等同于真实CPU完整时序。","review":"真实CPU还涉及过程映像、通信和中断，模型不用于推断实际响应时间。"},
