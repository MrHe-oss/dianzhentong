...
QUESTIONS = (
    ...
    _q(91,"p2_unit_1","负责执行用户程序的核心角色是？",("CPU","任意信号模块","所有外部按钮"),"CPU","CPU负责程序执行和系统协调。","CPU角色"),
    _q(92,"p2_unit_1","需要增加输入输出处理能力时，首先应考虑哪类角色？",("信号模块","项目标题","程序注释"),"信号模块","信号模块用于扩展输入输出能力。","信号扩展"),
    _q(93,"p2_unit_1","通信模块的主要功能是？",("扩展信息交换能力","替代CPU执行全部程序","直接完成真实接线"),"扩展信息交换能力","通信模块服务于控制系统的信息交换需求。","通信能力"),
    _q(94,"p2_unit_1","理解PLC硬件时更可靠的分类依据是？",("功能与任务需求","外壳颜色","网页排列位置"),"功能与任务需求","按功能角色理解比记忆外观更容易迁移。","硬件分类"),
    _q(95,"p2_unit_1","本平台中的PLC硬件内容可以用于什么？",("离线教学理解","直接选择真实设备型号","指导现场带电操作"),"离线教学理解","平台不提供选型、接线或真实设备操作依据。","安全边界"),
    _q(96,"p2_unit_2","PLC程序循环的抽象顺序是？",("读取输入—执行程序—更新结果","更新结果—拆线—读取输入","只执行一次程序"),"读取输入—执行程序—更新结果","PLC通常持续循环处理输入、程序和输出结果。","程序循环"),
    _q(97,"p2_unit_2","布尔变量适合表达哪类信息？",("真或假状态","任意长度文章","设备外观"),"真或假状态","布尔量用于表达二值逻辑状态。","数据类型"),
    _q(98,"p2_unit_2","程序设计前最适合先明确什么？",("控制要求与变量含义","真实端子接线","设备外观"),"控制要求与变量含义","明确需求与变量是组织可读逻辑的基础。","变量定义"),
    _q(99,"p2_unit_2","提高程序可读性的合理方法是？",("按目标拆分逻辑并清晰命名","把全部逻辑堆在一起","删除所有注释"),"按目标拆分逻辑并清晰命名","结构化网络、语义化变量和必要注释有助于检查。","逻辑结构"),
    _q(100,"p2_unit_2","抽象式Y(k+1)=F(X(k),S(k))主要表达什么？",("当前输入与状态影响下一轮结果","真实接线位置","CPU型号选择"),"当前输入与状态影响下一轮结果","公式概括信息依赖；这里旧题的‘下一轮结果’指本轮计算后的更新边界Y(k+1)，不是要求再等待一个完整循环。教学模型为本轮读取的信息—本轮计算—结果更新，不代表实际CPU完整时序。","循环关系"),
    _q(101,"p2_unit_3","TIA博途项目在工程中的主要作用是？",("统一组织工程对象","替代控制器硬件","自动证明程序正确"),"统一组织工程对象","项目是设备、程序块和变量等对象的组织容器。","工程项目"),
    _q(102,"p2_unit_3","设备组态主要描述什么？",("控制器与模块构成","学生个人信息","程序运行成绩"),"控制器与模块构成","设备组态建立工程中的硬件描述。","设备组态"),
    _q(103,"p2_unit_3","承载控制逻辑的主要工程对象是？",("程序块","项目标题","软件窗口颜色"),"程序块","程序块用于组织和承载控制逻辑。","程序块"),
    _q(104,"p2_unit_3","变量表的主要学习价值是？",("集中管理名称、类型和含义","替代程序逻辑","直接连接真实设备"),"集中管理名称、类型和含义","集中管理能提高变量定义的一致性。","变量管理"),
    _q(105,"p2_unit_3","硬件描述与程序逻辑之间的关系是？",("相互关联但职责不同","完全相同","互不需要"),"相互关联但职责不同","组态描述工程硬件，程序块表达控制逻辑。","工程对象关系"),
    _q(106,"p2_unit_4","简单PLC项目的合理起点是？",("需求与控制目标","随意堆叠指令","直接连接真实设备"),"需求与控制目标","需求决定变量、逻辑和验证目标。","需求分析"),
    _q(107,"p2_unit_4","编译通过能否证明控制需求完全正确？",("不能","一定能","与程序无关"),"不能","编译主要检查工程规则，逻辑正确性与安全性仍需单独验证。","编译检查"),
    _q(108,"p2_unit_4","发现变量含义不清时应先做什么？",("返回需求并明确变量定义","忽略变量继续操作","进行真实送电试验"),"返回需求并明确变量定义","清晰定义是后续逻辑检查的基础。","变量复核"),
    _q(109,"p2_unit_4","离线模拟发现逻辑异常时，合理做法是？",("回到需求和程序逻辑复盘","在真实设备上试探","强制改变真实变量"),"回到需求和程序逻辑复盘","平台只支持离线教学复盘。","模拟验证"),
...
QUESTIONS += (
    QuizQuestion("plc_cycle_snapshot", "p2_unit_2", "抽象演示本轮读取到假，读取后输入变为真。计算只使用本轮快照，结果更新时应显示什么？", ("假，下一轮重新读取后才可能改变", "真，外部变化会改写本轮快照", "立即跳过计算阶段"), "假，下一轮重新读取后才可能改变", "本轮计算仍使用读取时的假值，更新显示假；下一轮重新读取真值后才计算新结果。这是教学模型，不是实际CPU完整时序。", "本轮读取与结果更新"),
...
from .direction_logic import QUESTION_SPECS as DIRECTION_QUESTIONS, OPTION_FEEDBACK as DIRECTION_FEEDBACK
...
QUESTIONS += tuple(QuizQuestion(**{k: v for k, v in row.items() if k not in {"card", "wrong"}}) for row in DIRECTION_QUESTIONS)
...
QUESTION_CARD_MAP = {
    ...
    **{f"q{number:02d}": "sequence_control" for number in range(86, 91)},
    "q91": "plc_cpu", "q92": "plc_modules", "q93": "plc_modules",
    "q94": "plc_system_role", "q95": "plc_system_role",
    "q96": "plc_program_cycle", "q97": "plc_variables", "q98": "plc_variables",
    "q99": "plc_logic_structure", "q100": "plc_program_cycle",
    "q101": "tia_project", "q102": "tia_device_config", "q103": "tia_program_blocks",
    "q104": "tia_program_blocks", "q105": "tia_device_config",
...
QUESTION_CARD_MAP.update({"plc_cycle_snapshot": "plc_program_cycle", "plc_variable_meaning": "plc_variables", "plc_stop_priority": "plc_logic_structure"})

OPTION_FEEDBACK = {
    "q96": {"更新结果—拆线—读取输入": "拆线不是程序循环阶段，本模型先读取信息，再计算并更新结果。", "只执行一次程序": "循环意味着重复处理，不是只执行一次；这里仍是抽象教学顺序。"},
    "q97": {"任意长度文章": "布尔量只有真或假，不能保留文章内容。", "设备外观": "外观不是本题要表示的二值逻辑状态。"},
    "q99": {"把全部逻辑堆在一起": "堆叠逻辑不利于逐项核对条件，应按目标拆分。", "删除所有注释": "删除解释不能改善变量含义的理解，清晰命名和必要说明更有帮助。"},
    "q100": {"真实接线位置": "公式表达信息依赖，没有给出任何位置或接线信息。", "CPU型号选择": "该抽象关系没有容量和性能条件，不能用于型号选择。"},
    "plc_cycle_snapshot": {"真，外部变化会改写本轮快照": "混淆了外部输入与已读取快照。本题限定计算使用快照，后来的变化留待下轮读取。", "立即跳过计算阶段": "输入变化并不取消模型的计算阶段，本轮仍按快照计算后更新。"},
    "plc_variable_meaning": {"两者都用布尔量即可保留数量": "真或假无法区分1次和2次，数量信息会丢失。", "两者都只保存文字说明": "变量说明不等于可参与逻辑或数量处理的值，应分别定义状态和数量。"},
    "plc_stop_priority": {"真，启动优先于停止": "题设是停止优先：T为真使非T为假，即使A和B都真也不能成立。", "沿用上次结果": "题设没有上次状态变量，也没有自锁，不能添加保持假设。"},
    "hw_cpu_transfer": {
...
QUESTION_CARD_MAP.update({row["id"]: row["card"] for row in DIRECTION_QUESTIONS})
OPTION_FEEDBACK.update(DIRECTION_FEEDBACK)
QUESTION_CARD_MAP.update({row['id']: row['card'] for row in JOG_HOLD_QUESTIONS})
OPTION_FEEDBACK.update(JOG_HOLD_FEEDBACK)
QUESTION_CARD_MAP.update({row['id']: row['card'] for row in PLC_SD_QUESTIONS})
...
def textbook_question_pool(chapter_ids: Sequence[str], example_question_id: str | None = None) -> tuple[QuizQuestion, ...]:
    ...
    return tuple(q for q in QUESTIONS if q.chapter_id in chapter_ids
                 and not (q.chapter_id in {"p2_unit_1", "p2_unit_2", "p2_unit_3", "p2_unit_4", "p3_unit_1", "p3_unit_2", "p3_unit_3", "p3_unit_4"}
...
def make_quiz_record(chapter_id: str, answers: Sequence[QuizAnswer], mode: str = "chapter_quiz",
    ...
    total = len(frozen)
    threshold = 0.75 if mode == "textbook_project_assessment" else (0.7 if mode in {"course_exam", "textbook_unit_assessment"} else (pass_threshold if pass_threshold is not None else 0.6))
    return QuizRecord(
