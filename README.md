...
电诊通是面向电气工程及其自动化专业学生的电气知识与模拟实训学习平台。v2.7 为四门课程各增加一项综合实训任务，把情境分析、首次判断、错因解释、学习反思和报告下载组成完整训练。平台不提供真实端子、电压或接线指导，匿名学习档案继续兼容旧备份。
...
- 14个电气控制核心术语的集中解释；
- 4门课程、12个章节共75道教学题，每次章节测验随机抽取5题并即时解释；
...
- 引导学习模式、17张核心知识卡和不含真实接线信息的控制逻辑关系示意；
- 12个互动识图案例（含6个星—三角案例），记录首次判断得分、错误步骤和推荐路径；
- 星—三角课程提供“停止—星形启动—转换等待—三角运行”状态演示和专项学习总结；
- 四门课程各提供一项5步综合实训，完成学习反思后生成可下载的实训学习报告；
- 1分钟新手引导，说明实验选择、模拟资料阅读和三种判断；
- 17张知识卡、75道题、20类模拟故障和12个识图案例均显示可追溯来源；
