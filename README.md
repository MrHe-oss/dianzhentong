...
电诊通是面向电气工程及其自动化专业学生的电气知识与模拟实训学习平台。v1.4 在课程地图基础上增加30道固定规则章节题、即时解析、60%通过标准和错题优先复习，并保留两个电动机控制实验、每日任务、掌握度、知识卡和内容审校状态。
