...
- PLC测验与例题记录支持JSON导出、恢复及重复导入去重；教材跨章节评测仍按所在单元校验。
- 10分钟复习清单打开原错题；相似题巩固为独立入口，不替代原错题的连续两次正确判定。
- 例题进度只依据实际记录，不从知识点学习状态推断；例题练习不作为错题掌握的证据。
- 无数据库结构或备份格式升级；既有学习记录保留。

电诊通是面向电气工程及其自动化专业学生的教材知识学习平台。v4.4 为《电气控制与PLC应用技术（S7-1200）》项目2补齐学习闭环：4个PLC单元均具备原创知识小课、例题变式、单元测验、互动逻辑训练和掌握进度。项目与任务名称依据机械工业出版社公开目录，讲解与训练均由平台独立编写。识图、虚拟实训和故障诊断仅作为配套巩固工具，现有成绩与旧备份保持兼容。

> **仅用于教学模拟。** 不得用于真实设备诊断，不得替代持证电工、设备说明书或现场安全规程。本项目不提供带电测量、拆线、短接、送电或强制设备动作的指导。

## 功能范围

- 教材书架、教材内容全局搜索、知识点收藏和最近学习直达；
- 知识点个人笔记、统一复习本、笔记搜索与匿名TXT/JSON导出；
- 同一教材支持多个结构化项目；当前覆盖项目1基本电气线路控制与项目2初识S7-1200 PLC；
- 根据当前设备上的学习记录生成10分钟复习清单，连续两次正确后移除薄弱项；
- 学习中心显示待复习、已消除薄弱点、最近7天掌握及三类复习状态；
- 5分钟首次体验，不写入正式成绩，也不改变课程进度；
- 4门课程、12个章节的学习地图、完成状态与顺序解锁；
- 14个电气控制核心术语的集中解释；
- 5门课程、15个章节共90道教学题，每次章节测验随机抽取5题并即时解释；
...
- 引导学习模式、33张核心知识卡和不含真实接线信息的控制逻辑关系示意；
- 12个互动识图案例（含6个星—三角案例），记录首次判断得分、错误步骤和推荐路径；
- 星—三角课程提供“停止—星形启动—转换等待—三角运行”状态演示和专项学习总结；
- 五门课程各提供一项5步综合实训，完成学习反思后生成可下载的实训学习报告；
- 1分钟新手引导，说明实验选择、模拟资料阅读和三种判断；
- 33张知识卡、110道题、20类模拟故障和22个识图案例均显示可追溯来源；
...
- `dianzhentong/content/textbooks/`：结构化教材内容包；
- `dianzhentong/content_loader.py`：教材加载与启动校验；
